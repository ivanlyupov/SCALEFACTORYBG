...
### Пренасочване от стария `.vercel.app` адрес

Старият адрес `motioncraftbg.vercel.app` беше споделян публично, преди домейнът да
е свързан. В `vercel.json` има правило, което праща всеки, отворил стария адрес,
към `https://www.scalefactory-bg.com` (запазва и страницата, която е поискал).

- `"permanent": false` = временно пренасочване (307). Нарочно е така: постоянното
  (308) се запомня от браузърите завинаги и старият адрес би станал неизползваем,
  ако някога има проблем с домейна. Сменете на `true` само ако сте сигурни.
- ⚠ **Във `vercel.json` не може да има коментари.** Vercel проверява файла по строга
  схема и непознат ключ (напр. `"//"`) проваля деплоя — сайтът остава на старата
  версия, без видима грешка.
